...
// Upit na bazu koji uz pomoć SQL funkcije AVG računa prosječnu ocjenu o određenom doktoru
$ocjena = "SELECT ime_i_prezime, AVG(ocjena) AS ocjena_doktora FROM doktor d, doktor_ocjena do WHERE d.id = do.id_doktor GROUP BY ime_i_prezime ORDER BY AVG(ocjena) DESC";
$sr = izvrsiUpit($ocjena);

// Ispis tablice kao HTML elementa
echo "<table>";
echo "<th>Ime i prezime</th>";
echo "<th>Prosjecna ocjena</th>";
...
while (($row = mysqli_fetch_array($sr, MYSQLI_ASSOC)) != false) {
	...
	foreach ($ocjene as $data) {
		...
		echo "<td>" . $data['ocjena_doktora'] . "</td>";
		echo "</tr>";
